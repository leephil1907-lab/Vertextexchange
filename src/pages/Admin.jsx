    ...
    <div style={{ background: "#070b14", padding: "24px 14px" }}>
      <div style={{ maxWidth: 560, margin: "0 auto", borderRadius: 16, overflow: "hidden", border: "1px solid #1d2b47", background: "#0b1120" }}>
        ...
        <div style={{ padding: "22px 24px 6px" }}>
          ...
          {paras}
          {ctaLabel && <span style={{ display: "inline-block", padding: "11px 24px", borderRadius: 10, background: "#0d9e77", color: "#fff", fontWeight: 700, fontSize: 14 }}>{ctaLabel}</span>}
        </div>
        ...
        <div style={{ background: "#0d1526", borderTop: "1px solid #1d2b47", padding: "14px 24px", display: "flex", justifyContent: "space-between", fontSize: 12, color: "#6b7a94" }}>
          ...
          <span><span style={{ color: "#2dd4a7" }}>Support</span> · <span style={{ color: "#2dd4a7" }}>Email preferences</span> · <span style={{ color: "#2dd4a7" }}>Terminal</span></span>
        </div>
      </div>
    </div>
  );
        ...
        {!t ? <div className="empty-state">Select a ticket.</div> : (
          <>
            ...
            <div style={{ maxHeight: 340, overflowY: "auto", marginBottom: 14 }}>
              {t.messages.map((m, i) => (
                <div key={i} className={"msg " + m.from} style={{ margin: "0 0 10px " + (m.from === "support" ? "auto" : 0), maxWidth: "85%", padding: "10px 14px", borderRadius: 12, fontSize: 13.5, lineHeight: 1.55, background: m.from === "support" ? "rgba(45,212,167,.12)" : "var(--bg-elev)", border: "1px solid var(--line)", color: "var(--text)", textAlign: "left" }}>
